...
public class HuffmanCoding {
    ...
    public void makeSortedList() {
        ...
        for (int i = 0; i < 128; i++) {
            ...
            for (int j = 0; j < charArray.size(); j++) {
                if (charArray.get(j) == (char) i) {
                    freq++;
                }
            }
            if (freq > 0) {
                if (freq / charArray.size() == 1) {
                    CharFreq temp1 = new CharFreq();
                    int moop = i + 1;
                    if (moop > 127)
                        moop = 0;
                    temp1.setCharacter((char) moop);
                    temp1.setProbOcc(0);
                    sortedCharFreqList.add(temp1);
                }
                temp.setCharacter((char) i);
                temp.setProbOcc((double) freq / charArray.size());
    ...
    public void makeTree() {
        // 1. Start two empty queues: source + target
        Queue<CharFreq> source = new Queue<CharFreq>();
        Queue<TreeNode> target = new Queue<TreeNode>();

        /*
         * 2. new node for each character
         * 3. increasing order
         */
        ...
        while (!(source.isEmpty()) || target.size() != 1) {
            TreeNode node1 = getTreeNode(source, target);
            TreeNode node2 = getTreeNode(source, target);
            CharFreq combined = new CharFreq(null, node1.getData().getProbOcc() + node2.getData().getProbOcc());
            target.enqueue(new TreeNode(combined, node1, node2));
        }
    ...
    private TreeNode getTreeNode(Queue<CharFreq> source, Queue<TreeNode> target) {
        if (target.isEmpty()) {
        ...
        double sourceOcc = source.peek().getProbOcc();
        double targetOcc = target.peek().getData().getProbOcc();
        if (sourceOcc > targetOcc) {
            return target.dequeue();
        }
        return new TreeNode(source.dequeue(), null, null);
    }
    ...
    private void code(String str, TreeNode huffmanRoot) {
        TreeNode ptr = huffmanRoot;
        if (ptr.getLeft() != null)
            code(str + "0", ptr.getLeft());
        if (ptr.getRight() != null)
            code(str + "1", ptr.getRight());
        if (ptr.getRight() == null && ptr.getLeft() == null)
            encodings[(int) ptr.getData().getCharacter()] = str;
    }
    ...
    public void encode(String encodedFile) {
        ...
        while (StdIn.hasNextChar()) {
            str = str + encodings[(int) StdIn.readChar()];
        }
    ...
    public void decode(String encodedFile, String decodedFile) {
        ...
        while (str.length() != 0) {
            ...
            while (ptr.getData().getCharacter() == null) {
                if (str.substring(count, count + 1).equals("1"))
                    ptr = ptr.getRight();
                else
                    ptr = ptr.getLeft();
                count++;
            }
            decodedFile = decodedFile + ptr.getData().getCharacter();
